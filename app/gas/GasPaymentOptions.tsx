...
// Dummy icons (update with your real assets)
const upiApps = [
...
export default function GasPaymentOptions() {
    ...
    <View style={styles.container}>
      ...
      <ScrollView style={{ flex: 1 }} contentContainerStyle={{ paddingBottom: 90 }}>
        <View style={styles.boardBox}>
          ...
          <View style={{ flex: 1 }}>
            <Text style={styles.boardLabel}>{params.provider || "HP Gas"}</Text>
            <Text style={styles.boardSub}>Gas Cylinder</Text>
          </View>
          <Text style={styles.boardAmount}>₹ {params.billAmount || "920"}</Text>
          <TouchableOpacity style={styles.changeBtn} onPress={() => router.back()}>
            <Text style={styles.changeTxt}>Change Plan</Text>
          </TouchableOpacity>
        </View>
        ...
        <View style={styles.prefPayBox}>
          ...
          >
            <Icon
              name={selected === "merchant_upi" ? "radiobox-marked" : "radiobox-blank"}
              size={23}
              color={selected === "merchant_upi" ? "#00b894" : "#ccc"}
              style={{ marginRight: 8 }}
            />
            <Text style={styles.prefPayTitle}>Merchant 1-Click UPI</Text>
          ...
          <TouchableOpacity style={styles.paySingleClickBtn}>
            <Text style={styles.paySingleClickBtnTxt}>
              Pay ₹ {params.billAmount || "920"} In Single Click
            </Text>
          </TouchableOpacity>
        </View>
        ...
        <View style={styles.upiBox}>
          {upiApps.map((app, idx) => (
            ...
            >
              <Icon
                name={selected === "upi" && selectedUpi === idx ? "radiobox-marked" : "radiobox-blank"}
                size={22}
                color={selected === "upi" && selectedUpi === idx ? "#00b894" : "#bbb"}
      ...
      <View style={styles.footerBar}>
        <Text style={styles.footerAmt}>₹ {params.billAmount || "920"}</Text>
        <TouchableOpacity
          style={[
            styles.footerBtn,
          ...
          disabled={!canProceed}
          onPress={() =>
            canProceed && router.push({
              pathname: "/recharge/RechargeSuccess",
              params: {
...
const styles = StyleSheet.create({
  ...
  footerBtnDisabled: { backgroundColor: "#d3d5da" },
  footerBtnTxt: { color: "#fff", fontWeight: "bold", fontSize: 15 },
  footerBtnTxtDisabled: { color: "#aaa" }
});
